...
class BTCPredictor:
    ...
    def quick_predict(self):
        try:
            # to load existing model if available
            ...
            # hourly data first, daily if it fails
            try:
            ...
            # get current price and properly convert to float
    ...
    def full_analysis(self):
        ...
        try:
            ...
            # gets current price and properly convert to float
            current_price = df['Close'].iloc[-1]
            current_price_float = float(
                current_price.iloc[0]) if isinstance(current_price, pd.Series) else float(current_price)
            percent_change = ((predicted_price - current_price_float) /
